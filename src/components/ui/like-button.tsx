    ...
    <div className='my-12 flex justify-center'>
      <button
        ...
        className={cn([
          ...
          'before:absolute before:inset-0 before:rounded-full before:bg-gradient-to-br before:from-aired before:to-aired before:content-[""]'
        ])}
        type='button'
        onClick={handleLike}
      ...
      >
        <motion.span
          className='absolute inset-0 rounded-lg bg-gradient-to-br from-aired to-aired blur-xl'
          animate={{ scale: isBreathing ? scale : 1.5 }}
          transition={{ duration: 1, ease: 'linear' }}
        />
        <span
          className={cn([
            'absolute text-white inset-0.5 z-10 flex items-center justify-center gap-2 rounded-md bg-background text-lg font-bold transition-[background-color] duration-150',
            'group-hover:bg-transparent group-hover:text-background dark:group-hover:text-foreground'
          ])}
        >
          <Icons.heart
            className={cn(
              'w-7  group-hover:fill-background dark:group-hover:fill-foreground',
              data &&
