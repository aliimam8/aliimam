...
import type { AssetsPost, DocumentTypes } from 'contentlayer/generated';
...
export function sortedBlogPost(allAssets: AssetsPost[]) {
    return allAssets.sort((a, b) => dateSortDesc(a.date, b.date)).filter((p) => p.draft === false);
  }
  ...
  export function getAllTags(allBlogs: AssetsPost[]) {
  ...
  export function formatBlogLink(
    blog: Omit<AssetsPost, 'body' | '_raw' | '_id'> | null
  ): BlogLink | undefined {
    if (blog) {
      return {
        title: blog?.title,
