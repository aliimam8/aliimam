 ...
 * @returns {array}
 */
export const hslToRgb = (hue: HSLColour | number, sat?: number, lum?: number): number[] => {
  let h: number;
  let s: number;
  let l: number;
 ...
 */
export const rgbToHex = (red: number | number[], green: number, blue: number) => {
  let r;
  let g;
  let b;
...
export const hexToRgb = (hex: string): number[] => {
  ...
  switch (formattedHex.length) {
    case 3: {
      const ret = formattedHex.split('').map((item: any) => parseInt(item + item, 16));
      return ret;
    }

    case 6: {
      const ret = formattedHex.match(/.{2}/g).map((item: string) => parseInt(item, 16));
      return ret;
    }
...
export const hexToHsl = (hex: string) => {
  ...
  switch (formattedHex.length) {
    case 3:
      rgb = formattedHex.split('').map((item: any) => parseInt(item + item, 16));
      break;
    case 6:
      rgb = formattedHex.match(/.{2}/g).map((item: string) => parseInt(item, 16));
      break;
    default:
      return [];
  }
...
export const hslToCss = (h: number, s: number, l: number) => `hsl(${h},${s}%,${l}%)`;
